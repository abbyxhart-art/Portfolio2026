import svgPaths from "./svg-9s5re4pift";
type CursorProps = {
  className?: string;
  instance?: "Default" | "External Links" | "Internal Page Hover" | "Shoutout" | "External Link" | "Internal Link";
...
function Cursor({ className, instance = "Shoutout", type = "Default" }: CursorProps) {
  ...
  const isHoverAndIsInternalLinkOrExternalLink = type === "Hover" && ["Internal Link", "External Link"].includes(instance);
  ...
  return (
    <div className={className || `relative ${isExternalLinkAndHover ? "bg-[#e5cfff] h-[28px] rounded-[16px] w-[90px]" : isInternalLinkAndHover ? "bg-[#faf9ff] h-[28px] rounded-[16px] w-[90px]" : isExternalLinksAndCllick ? "bg-[rgba(154,71,255,0.3)] rounded-[18.667px] size-[28px]" : isExternalLinksAndDefault ? "bg-[rgba(154,71,255,0.1)] rounded-[18.667px] size-[28px]" : isInternalPageHoverAndClick ? "bg-[rgba(35,34,38,0.2)] rounded-[18.667px] size-[28px]" : isInternalPageHoverAndDefault ? "bg-[rgba(35,34,38,0.1)] overflow-clip rounded-[18.667px] size-[28px]" : isDefaultAndDefault ? "bg-[rgba(200,196,215,0.2)] rounded-[18.667px] size-[28px]" : isShoutoutAndClick ? "size-[32px]" : "size-[28px]"}`}>
      ...
      {(isShoutoutAndDefault || isShoutoutAndClick || isInternalLinkAndHover || isExternalLinkAndHover) && (
        <div className={`absolute ${isHoverAndIsInternalLinkOrExternalLink ? "left-0 size-[24px] top-0" : isShoutoutAndClick ? "flex h-[3.987px] items-center justify-center left-[21.33px] top-[8px] w-[0.32px]" : "flex h-[3.489px] items-center justify-center left-[18.67px] top-[7px] w-[0.28px]"}`} style={isShoutoutAndIsDefaultOrClick ? { "--transform-inner-width": "1200", "--transform-inner-height": "19" } : (undefined as React.CSSProperties)}>
          ...
          {isHoverAndIsInternalLinkOrExternalLink && (
      ...
      {isHoverAndIsInternalLinkOrExternalLink && <p className={`absolute font-["Inter:Regular",sans-serif] font-normal leading-[1.2] not-italic text-[14px] text-black top-[5px] whitespace-nowrap ${isExternalLinkAndHover ? "left-[calc(50%-37px)]" : "left-[8px]"}`}>{isExternalLinkAndHover ? "Instagram" : isInternalLinkAndHover ? "2 min read" : ""}</p>}
    </div>
  );
}

export default function Cursor1() {
  return <Cursor className="bg-[#faf9ff] relative rounded-[16px] size-full" instance="Internal Link" type="Hover" />;
}